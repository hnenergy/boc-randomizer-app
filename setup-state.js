(function (root, factory) {
  ...
  const STORAGE_KEY = 'spinorder-setup-v1';
  const VERSION = 1;
  ...
  const DEFAULT_VALUES = Object.freeze({eventName: '', activity: 'Football', activityLabel: 'Random Order', customLabel: '', spinMode: 'manual'});
  ...
  function load(storage) {
    try {
      ...
      if (!saved || saved.version !== VERSION || !saved.values || typeof saved.values !== 'object') return {...DEFAULT_VALUES};
  ...
  return {STORAGE_KEY, VERSION, ACTIVITIES, LABELS, DEFAULT_VALUES, normalize, validate, displayLabel, save, load};
});
