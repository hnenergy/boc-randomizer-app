...
The current static PWA opens on a responsive SpinOrder landing page, collects event settings, accepts 2–20 manually entered names or 2–60 names from a local `.txt`/`.csv` import, and then creates the randomizer. Terminology choices are Draft Order, Random Order, Drawing Order, or a custom label. Manual mode spins once per click; Auto Spin starts immediately and continues after a visible three-second countdown with Pause, Resume, and Stop controls. Each spin begins at the wheel's previous landing angle and adds its animation rotation cumulatively. A selected wedge stays at its landing point while idle or counting down, then leaves only when the next spin starts; it is already excluded from selection. A DPR-aware local canvas redraws eligible wedges and centered labels while preserving cumulative orientation. Setup can be edited without losing results unless the reveal direction changes, which requires confirmation before resetting the assignments. In either spin mode, when only one participant and one position remain, SpinOrder holds the penultimate result for two seconds, then completes the deterministic final assignment without another click, countdown, wheel animation, or sound and redraws the completed wheel with only that final participant. Current Position and Selected Position stay synchronized during the event and display `Order Set` when every position is assigned. Home provides a confirmed start-over that clears only the active SpinOrder event. Example names are available on request as `Team 1` through `Team 12` but are never loaded automatically.
